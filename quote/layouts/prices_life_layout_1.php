...
var dValue = Slider_Values.indexOf(<?php echo $_SESSION['duration'];?>);
...
<div class="row filterdiv hidden-xs" style="border: 1px solid #ddd;text-align: center;padding-top: 10px;border-top: 4px solid #DDD; margin-bottom:20px; background:#FFF;">	
	...
	<div class="col-md-3 adjust-quoto" style="border:none;">
                <h4 class="deductible" style="margin: 0;padding: 0;font-weight: bold;margin-bottom: 0;border: none;text-align: left;">Duration: <input type="text" id="coverage_deductible" name="coverage_deductible" value="<?php echo $_SESSION['duration'];?> year(s)" style="border:0; font-size:24px; color:#444; font-weight:bold;background: no-repeat;margin: 0;padding: 0;text-align: center;width: 100px;"></h4>
	...
	<div class="col-md-3">
	<h3 style="margin: 0;">Show Prices</h3>
		<label class="radio">
			<input type="radio" name="radio-btn" value="1" checked="checked" onclick="$('.ratebase-1').show();$('.ratebase-2').hide();">
			<i></i> Monthly
		</label>
		<label class="radio">
			<input type="radio" name="radio-btn" value="2" checked="checked" onclick="$('.ratebase-2').show();$('.ratebase-1').hide();">
			<i></i> Yearly
		</label>
...
<div class="row">
		...
		$plans_q = mysqli_query($db,  "SELECT * FROM `wp_dh_life_plans` WHERE `product`='$product_id' AND `smoke`='$smoke' AND `status`='1' ORDER BY `id`" );
		...
		$deductsloop = mysqli_query($db,  "SELECT `duration` FROM `wp_dh_life_plans_rates` WHERE `plan_id` IN (SELECT `id` FROM wp_dh_life_plans WHERE `product`='$product_id') GROUP BY `duration` ORDER BY CAST(`duration` AS DECIMAL)" );
		while($deductsloop_f = mysqli_fetch_assoc($deductsloop)){
		$duration = $deductsloop_f['duration'];
		
		$sum_insured= '';
		$sumin = mysqli_query($db, "SELECT `sum_insured` FROM `wp_dh_life_plans_rates` WHERE `plan_id` IN (SELECT `id` FROM wp_dh_life_plans WHERE `product`='$product_id') GROUP BY `sum_insured` ORDER BY CAST(`sum_insured` AS DECIMAL)");
		...
		//getting prices for each ages
			...
			$display = array();
					...
					$plan_rates = mysqli_query($db, "SELECT * FROM `wp_dh_life_plans_rates` WHERE `plan_id`='$plan_id' AND `sex`='$gender' AND '$person_age' BETWEEN `minage` AND `maxage` AND `sum_insured`='$sum_insured'" );	
...
<div class="listing-item ratebase-<?php echo $rate_base;?>" data-listing-price="<?php echo str_replace(',', '', number_format($total_price));?>">
...
<div class="row plan-details   deductable-<?php echo $duration; ?>" style="display: <?php if($_SESSION['duration'] == $duration){ echo 'block'; } else { echo 'none'; } ?>; margin-top:0; margin-left:0; margin-right:0; margin-bottom 5px !important;border-bottom: 1px solid #0084c1;">
	<div class="col-md-3 col-xs-6 text-center" style="padding-top: 20px;">
	<img src="admin/uploads/<?php echo $comp_logo ?>"/>
	</div>
	...
	<?php echo $duration;?> Year(s) Duration</div>
	<div class="col-md-4 col-xs-12 text-center" style="border:2px solid #f5821f; padding:10px;box-shadow: 0px 0px 5px 0px #999 inset;">
		...
		<h1 style="padding: 0;margin: 0;line-height: normal;">$<?php echo str_replace(',','', number_format($total_price,2));?>
		<small style="color: #f5821f;font-weight: bold;font-size: 13px;margin-left: -9px;"><?php if($rate_base == '1'){ echo '/Month'; } else { echo '/Year'; } ?></small></h1>
		
		<small>1 Insurer(s)</small>
		</div>
		<div class="col-md-4 col-xs-6" style="padding-top: 10px;padding-left: 0;padding-right: 0;">
<style>
.hoverdetails_<?php echo $duration.$plan_id;?> {
...
.hoverdetails_<?php echo $duration.$plan_id;?> h2 {
...
.hoverdetails_<?php echo $duration.$plan_id;?> h3 {
font: 400 14px Arial,Helvetica,sans-serif;line-height: normal;color: #333;line-height: 30px;padding-bottom: 5px;margin: 0;	
}
.hoverdetails_<?php echo $duration.$plan_id;?> h3 span{
color: #ff8400;
font-weight:bold;	
}
.hover_<?php echo $duration.$plan_id;?>:hover  .hoverdetails_<?php echo $duration.$plan_id;?>{
	display:block !important;
}

</style>		
			<ul style="padding: 0;">
				<li style="list-style: none;" class="hover_<?php echo $duration.$plan_id;?>"><a href="#"> Policy Details</a>
				
				<div class="row hoverdetails_<?php echo $duration.$plan_id;?>" >
					...
					<div class="col-md-12 no-padding"><small>Total: <span style="color: #f5821f;">$<?php echo number_format($total_price,2);?></span></small></div>
					<div class="col-md-12 no-padding"><small>Duration: <span style="color: #f5821f;"><?php echo $duration;?> Year(s)</span></small></div>
					<div class="col-md-12 no-padding">
					<?php
					$person_price = $total_price;
					?>
...
$buynow_url = "life_buy.php?email=$email&coverage=".$sum_insured."&traveller=".$number_travelers."&duration=".$duration."&deductible_rate=$deduct_rate&person1=$date_of_birth&days=$num_of_days&companyName=$comp_name&comp_id=".$comp_id."&planname=".$plan_name."&plan_id=".$plan_id."&tripdate=$startdate&tripend=$enddate&premium=$total_price&destination=$destination&cdestination=&product_name=$product_name&product_id=$product_id&country=$primary_destination&visitor_visa_type=$product_name&tripduration=$num_of_days&age=$ages_array[0]&dob=$dob&agent=$agent&broker=$broker";
?>
	<div class="compare col-md-3 col-xs-12 text-center buttons_block"><a class="submit-btn col-md-12 col-xs-5 " onclick="$('.buynow_<?php echo $duration.$plan_id;?>').fadeIn();" style="font-weight: bold;padding: 6px 20px;font-size: 16px;display: block;color: #FFF; margin-bottom:5px;margin-top: 10px;border-radius: 6px;border-bottom: 2px solid #999;box-shadow: none;"><i class="fa fa-shopping-cart"></i> Buy Now</a>
	...
	<div class="row buynow_<?php echo $duration.$plan_id;?>" style="clear:both;margin: 0;border: 1px solid #CCC; display:none;">
	...
	<p><input type="checkbox" name="agree" style="height: auto;margin: 0;"> I give permission to LifeAdvice.ca to transfer my quote information and contact details to <?php echo $comp_name;?> in order to complete the purchase of travel insurance. LifeAdvice values your privacy. For details, see our <a href="/">Privacy Policy</a></p>
		<p></p>
	<p><a class="submit-btn" href="<?php echo $buynow_url;?>" style="font-weight: bold;padding: 6px 20px;font-size: 16px;display: block;color: #FFF; margin-bottom:5px;margin-top: 10px;border-radius: 6px;border-bottom: 2px solid #999;box-shadow: none;"><i class="fa fa-shopping-cart"></i> Buy Now</a></p>
	</div>
	<div class="col-md-6 text-center" style="font-size:16px;">
	<a href="#" onclick="$('.buynow_<?php echo $duration.$plan_id;?>').fadeOut();" class="pull-right text-danger" style="font-size:16px;"><i class="fa fa-close"></i></a>
	<p>or</p>
	<p>BY CALLING</p>
